...
class BigIntegrationLLMPos2Test : public ::testing::Test {
public:
...
std::string BigIntegrationLLMPos2Test::g_data_base = "";

TEST_F(BigIntegrationLLMPos2Test, LLMAfterFirstPaperAttribute) {
    ...
    std::string llm_cfg = R"({"provider":"demo","model":"demo","model_name":"demo_model",)"
                          R"("tuple_format":"FTREE","context_column":{"ATitle":64,"AAbs":64},"prompt":"PROMPT"})";
    ...
    // LLM query
    std::vector<std::string> ord_llm = {"ATitle", "AAbs", "llm_out", "BTitle", "BAbs", "CTitle", "CAbs"};
    ...
    std::cout << "LLM Pos2 - Plain: " << count_plain << " LLM: " << count_llm << std::endl;
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg.find("--data_dir=") == 0) { ffx::BigIntegrationLLMPos2Test::g_data_base = arg.substr(11); }
    }
