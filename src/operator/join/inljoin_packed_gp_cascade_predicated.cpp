...
template<typename T>
static void register_node_in_saved_data_gp_pred(FtreeStateUpdateNode* node, VectorGPSavedData<T>* vector_saved_data,
                                                uint32_t& saved_data_index);
template<typename T>
static void register_backward_nodes_in_saved_data_gp_pred(FtreeStateUpdateNode* parent_node,
                                                          VectorGPSavedData<T>* vector_saved_data,
                                                          uint32_t& saved_data_index);

template<typename T>
void INLJoinPackedGPCascadePredicated<T>::create_slice_update_infrastructure(const FactorizedTreeElement* ftree_leaf) {
    ...
    register_backward_nodes_in_saved_data_gp_pred<T>(_range_update_tree.get(), _vector_saved_data.get(),
                                                     saved_data_index);
    ...
    while (ftreenode != nullptr) {
        ...
        register_node_in_saved_data_gp_pred<T>(child_ptr, _vector_saved_data.get(), saved_data_index);
        child_ptr->parent = current_node;
        current_node->children.push_back(std::move(child));
        child_ptr->fill_bwd(ftreenode, _output_key);
        register_backward_nodes_in_saved_data_gp_pred<T>(child_ptr, _vector_saved_data.get(), saved_data_index);
...
template<typename T>
void INLJoinPackedGPCascadePredicated<T>::init(Schema* schema) {
    static_assert(std::is_same_v<T, uint64_t>, "INLJoinPackedGPCascadePredicated currently only supports uint64_t.");
    auto& map = *schema->map;
    ...
    std::cout << "INLJoinPackedGPCascadePredicated(" << _join_key << "->" << _output_key << ")"
...
template<typename T>
void INLJoinPackedGPCascadePredicated<T>::store_slices() {
...
template<typename T>
void INLJoinPackedGPCascadePredicated<T>::restore_slices() {
    for (std::size_t i = 0; i < _vector_saved_data_count; i++) {
        auto& vec_data = _vector_saved_data[i];
        const Vector<T>* vec = vec_data.vector;
...
template<typename T>
void INLJoinPackedGPCascadePredicated<T>::execute() {
    num_exec_call++;
    store_slices();
...
INLJoinPackedGPCascadePredicated<T>::process_data_chunk(BitMask<State::MAX_VECTOR_SIZE>* _current_ip_mask,
...
static void register_backward_nodes_in_saved_data_gp_pred(FtreeStateUpdateNode* parent_node,
    ...
    for (const auto& child: parent_node->children) {
        register_node_in_saved_data_gp_pred<T>(child.get(), vector_saved_data, saved_data_index);
        register_backward_nodes_in_saved_data_gp_pred<T>(child.get(), vector_saved_data, saved_data_index);
    }
}

template class INLJoinPackedGPCascadePredicated<uint64_t>;
