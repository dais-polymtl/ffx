...
template<typename T>
inline void copy_values_cascade(T* dest, const T* src, size_t count) {
...
template<typename T>
static void register_node_in_saved_data(FtreeStateUpdateNode* node, VectorSavedData<T>* vector_saved_data,
                                        uint32_t& saved_data_index);
template<typename T>
static void register_backward_nodes_in_saved_data(FtreeStateUpdateNode* parent_node,
                                                  VectorSavedData<T>* vector_saved_data, uint32_t& saved_data_index);

template<typename T>
void INLJoinPackedCascade<T>::create_slice_update_infrastructure(FactorizedTreeElement* ftree_leaf) {
...
template<typename T>
void INLJoinPackedCascade<T>::init(Schema* schema) {
    static_assert(std::is_same_v<T, uint64_t>, "INLJoinPackedCascade currently only supports uint64_t. String support "
    ...
    if (resolved.source_table) {
        std::cout << "INLJoinPackedCascade " << _join_key << "->" << _output_key
...
template<typename T>
void INLJoinPackedCascade<T>::store_slices() {
...
template<typename T>
void INLJoinPackedCascade<T>::print_upstream_vectors() {
    for (std::size_t i = 0; i < _vector_saved_data_count; i++) {
        // auto& vec_data = _vector_saved_data[i];
...
template<typename T>
void INLJoinPackedCascade<T>::restore_slices() {
...
template<typename T>
void INLJoinPackedCascade<T>::execute() {
    num_exec_call++;
    ...
    for (auto in_vector_idx = static_cast<int32_t>(next_set_bit_in_range(*_active_mask_uptr, in_vec_start_idx,
         ...
         in_vector_idx = static_cast<int32_t>(
            ...
            if (output_elems_produced == 0) {
                ...
                assert(_invalidated_count <= State::MAX_VECTOR_SIZE);
                ...
                copy_values_cascade<T>(&out_vals[out_vector_write_idx], &adj_list.values[out_values_read_idx],
                ...
                if (out_vector_write_idx == State::MAX_VECTOR_SIZE) {
                    ...
                    if (num_second_stage_count > 0) {
                        ...
                        for (auto i = 0; i < num_second_stage_count; ++i) {
                            copy_values_cascade<T>(&out_vals[0], &adj_list.values[out_values_read_idx],
...
INLJoinPackedCascade<T>::process_data_chunk(BitMask<State::MAX_VECTOR_SIZE>* _current_ip_mask,
...
template class INLJoinPackedCascade<uint64_t>;
// template class INLJoinPackedCascade<ffx_str_t>; // TODO: Enable when hash-based adjacency list indexing is implemented
