...
template<typename T>
static void register_node_in_saved_data_pred(FtreeStateUpdateNode* node,
                                             VectorSliceUpdateSavedData<T>* vector_saved_data,
                                             uint32_t& saved_data_index);
template<typename T>
static void register_backward_nodes_in_saved_data_pred(FtreeStateUpdateNode* parent_node,
                                                       VectorSliceUpdateSavedData<T>* vector_saved_data,
                                                       uint32_t& saved_data_index);

template<typename T>
void INLJoinPackedPredicated<T>::create_slice_update_infrastructure(FactorizedTreeElement* ftree_leaf) {
    ...
    register_backward_nodes_in_saved_data_pred<T>(_range_update_tree.get(), _vector_saved_data.get(), saved_data_index);
    ...
    while (ftreenode != nullptr) {
        ...
        register_node_in_saved_data_pred<T>(child_ptr, _vector_saved_data.get(), saved_data_index);
        child_ptr->parent = current_node;
        current_node->children.push_back(std::move(child));
        child_ptr->fill_bwd(ftreenode, _output_key);
        register_backward_nodes_in_saved_data_pred<T>(child_ptr, _vector_saved_data.get(), saved_data_index);
...
template<typename T>
void INLJoinPackedPredicated<T>::init(Schema* schema) {
    ...
    std::cout << "INLJoinPackedPredicated(" << _join_key << "->" << _output_key << ")"
...
template<typename T>
void INLJoinPackedPredicated<T>::store_slices() {
...
template<typename T>
void INLJoinPackedPredicated<T>::restore_slices() {
    for (std::size_t i = 0; i < _vector_saved_data_count; i++) {
        auto& vec_data = _vector_saved_data[i];
        const Vector<T>* vec = vec_data.vector;
...
template<typename T>
void INLJoinPackedPredicated<T>::execute() {
    num_exec_call++;
    store_slices();
    ...
    for (auto in_vector_idx = static_cast<int32_t>(next_set_bit_in_range(*_active_mask_uptr, in_vec_start_idx,
         ...
         in_vector_idx = static_cast<int32_t>(
            ...
            // If all values across all chunks filtered out, clear the bit
            if (!any_passed) {
                CLEAR_BIT(*_active_mask_uptr, in_vector_idx);
                _invalidated_indices[_invalidated_count++] = in_vector_idx;
                continue;
...
INLJoinPackedPredicated<T>::process_data_chunk(BitMask<State::MAX_VECTOR_SIZE>* _current_ip_mask,
...
static void register_backward_nodes_in_saved_data_pred(FtreeStateUpdateNode* parent_node,
    ...
    for (const auto& child: parent_node->children) {
        register_node_in_saved_data_pred<T>(child.get(), vector_saved_data, saved_data_index);
        register_backward_nodes_in_saved_data_pred<T>(child.get(), vector_saved_data, saved_data_index);
    }
}

template class INLJoinPackedPredicated<uint64_t>;
// Note: INLJoinPackedPredicated<ffx_str_t> not instantiated - joins work with integer node IDs
