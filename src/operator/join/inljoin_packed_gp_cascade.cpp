...
template<typename T>
inline void copy_values_gp(T* dest, const T* src, size_t count) {
...
template<typename T>
static void register_node_in_saved_data(FtreeStateUpdateNode* node, VectorGPSavedData<T>* vector_saved_data,
                                        uint32_t& saved_data_index);
template<typename T>
static void register_backward_nodes_in_saved_data(FtreeStateUpdateNode* parent_node,
                                                  VectorGPSavedData<T>* vector_saved_data, uint32_t& saved_data_index);

template<typename T>
void INLJoinPackedGPCascade<T>::create_slice_update_infrastructure(const FactorizedTreeElement* ftree_leaf) {
...
template<typename T>
void INLJoinPackedGPCascade<T>::init(Schema* schema) {
    static_assert(std::is_same_v<T, uint64_t>, "INLJoinPackedGPCascade currently only supports uint64_t. String "
    ...
    if (resolved.source_table) {
        std::cout << "INLJoinPackedGPCascade " << _join_key << "->" << _output_key
...
template<typename T>
void INLJoinPackedGPCascade<T>::store_slices() {
...
template<typename T>
void INLJoinPackedGPCascade<T>::print_upstream_vectors() {
    for (std::size_t i = 0; i < _vector_saved_data_count; i++) {
        // auto& vec_data = _vector_saved_data[i];
...
template<typename T>
void INLJoinPackedGPCascade<T>::restore_slices() {
...
template<typename T>
void INLJoinPackedGPCascade<T>::execute() {
    num_exec_call++;
    ...
    uint16_t* RESTRICT in_offset = in_state->offset;
    ...
    for (auto gp_idx = static_cast<int32_t>(
        ...
        // Calculate the overlap between the grandparent range and input vector range
        const auto pstart_idx =
                std::max(static_cast<int32_t>(in_offset[gp_idx]), static_cast<int32_t>(in_vec_start_idx));
        const auto pend_idx =
                std::min(static_cast<int32_t>(in_offset[gp_idx + 1] - 1), static_cast<int32_t>(in_vec_end_idx));

        // Count active children for THIS grandparent
        uint32_t parent_count = 0;

        // If this grandparent is at the boundary of the input vector, we need to count manually
        // because the RLE might include entries outside parent range
        if (bool is_first_or_last = (gp_idx == gp_start_pos) || (gp_idx == gp_end_pos)) {
            ...
            // If it's also the last block, apply end mask too
            ...
            // Last block (only if different from first)
        ...
        assert(parent_count <= State::MAX_VECTOR_SIZE);
        for (auto in_vector_idx = static_cast<int32_t>(next_set_bit_in_range(*_active_mask_uptr, pstart_idx,
             ...
             in_vector_idx = static_cast<int32_t>(
                ...
                if (output_elems_produced == 0) {
                    ...
                    // Process the adjacency list elements
                    ...
                    copy_values_gp<T>(&out_vals[out_vector_write_idx], &adj_list.values[out_values_read_idx],
                    ...
                    if (out_vector_write_idx == State::MAX_VECTOR_SIZE) {
                        ...
                        const auto remaining_adj_list_elements =
                                output_elems_produced - elements_to_copy_in_first_stage;
                        ...
                        // Second stage: process middle elements that fill complete vectors
                        if (num_second_stage_count > 0) {
                            SET_START_POS(*in_state, in_vector_idx);
                            SET_END_POS(*in_state, in_vector_idx);
                            for (auto i = 0; i < num_second_stage_count; ++i) {
                                copy_values_gp<T>(&out_vals[0], &adj_list.values[out_values_read_idx],
                        ...
                        // Third stage: process remaining elements
        ...
        // After processing ALL children of this grandparent, check if it became empty
        //_gp_val_empty = parent_count == 0;
        _invalid_gp_indices[_invalid_gp_indices_cnt] = gp_idx;
        _invalid_gp_indices_cnt += parent_count == 0;
    }

    // Process any remaining data
    if (out_vector_write_idx > 0) {
        process_data_chunk(&_current_ip_mask, out_vector_write_idx - 1);
        out_vector_write_idx = 0;// Reset for next grandparent
    }
...
INLJoinPackedGPCascade<T>::process_data_chunk(BitMask<State::MAX_VECTOR_SIZE>* _current_ip_mask,
...
template class INLJoinPackedGPCascade<uint64_t>;
// template class INLJoinPackedGPCascade<ffx_str_t>; // TODO: Enable when hash-based adjacency list indexing is implemented
