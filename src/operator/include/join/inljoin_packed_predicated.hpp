#ifndef VFENGINE_INDEX_NESTED_LOOP_JOIN_PACKED_PREDICATED_OPERATOR_HH
#define VFENGINE_INDEX_NESTED_LOOP_JOIN_PACKED_PREDICATED_OPERATOR_HH
...
class INLJoinPackedPredicated final : public Operator {
public:
    INLJoinPackedPredicated() = delete;
    INLJoinPackedPredicated(const INLJoinPackedPredicated&) = delete;

    // Constructor with predicate expression
    INLJoinPackedPredicated(std::string join_key, std::string output_key, bool is_join_index_fwd,
...
using INLJoinPackedPredicatedUint64 = INLJoinPackedPredicated<uint64_t>;
using INLJoinPackedPredicatedString = INLJoinPackedPredicated<ffx_str_t>;

}// namespace ffx

#endif// VFENGINE_INDEX_NESTED_LOOP_JOIN_PACKED_PREDICATED_OPERATOR_HH
