#ifndef VFENGINE_INDEX_NESTED_LOOP_JOIN_PACKED_CASCADE_PREDICATED_OPERATOR_HH
#define VFENGINE_INDEX_NESTED_LOOP_JOIN_PACKED_CASCADE_PREDICATED_OPERATOR_HH
...
class INLJoinPackedCascadePredicated final : public Operator {
public:
    INLJoinPackedCascadePredicated() = delete;
    INLJoinPackedCascadePredicated(const INLJoinPackedCascadePredicated&) = delete;

    // Constructor with predicate expression
    INLJoinPackedCascadePredicated(std::string join_key, std::string output_key, bool is_join_index_fwd,
...
using INLJoinPackedCascadePredicatedUint64 = INLJoinPackedCascadePredicated<uint64_t>;
using INLJoinPackedCascadePredicatedString = INLJoinPackedCascadePredicated<ffx_str_t>;
