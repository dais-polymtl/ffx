...
inline std::unordered_map<std::string, TupleFormat> TUPLE_FORMAT = {
        ...
        {"COLUMN_ENCODED", TupleFormat::ColumnEncoded},
        {"FTREE", TupleFormat::CompactRepFormat}};
...
constexpr auto META_PROMPT =
        ...
        "- In **FTREE** mode (input starting with `[FTREE]`), the table data is provided as a **factorized tree representation**:\n"
        "  - Each `[ROOT b=...]` block describes one root key and its connected values.\n"
        "  - Nested attribute sections (e.g., `a:`, `c:`) list the distinct values reachable under that root.\n"
        "  - You must reason over **all implicit tuple combinations** induced by this tree, not just the values shown once.\n"
        "  - Let `N` be the value shown in `The Number of Tuples to Generate Responses for`. Your JSON response **must** be an object of the form `{ \"items\": [...] }` where `items` is an array of length exactly `N`, and each element corresponds to a single logical tuple (e.g., one `(a,b,c)` combination) with no extra commentary.\n"
        "\n"
        "## Instructions\n"
        "```\n"
        "{{INSTRUCTIONS}}\n"
